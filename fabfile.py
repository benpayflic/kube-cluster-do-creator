...
def install_kubernetes(ctx):
    ...
    ctx.sudo("apt-mark hold kubelet kubeadm kubectl kubernetes-cni")
...
def configure_master(ctx):
    ...
    """
    ctx.sudo("kubeadm init --pod-network-cidr=10.244.0.0/16")
...
def get_nodes(ctx):
    for conn in get_connections(hosts):
        ...
        conn.sudo("curl -k https://localhost:6443/livez?verbose")

@task
def output_kube_config(ctx):
    """Output kube config for local access to cluster"""
    for conn in get_connections(hosts):
        print("++++++ Copy the below output and paste into kube config on local machine ++++++")
        conn.sudo("cat ~/.kube/config")